...
#=======> INLINING STARTS: ../common_utilities.sh
...
#=======> INLINING ENDS: ../common_utilities.sh

#=======CONFIGURATIONS=========================================================

MATHESAR_PORT=8000

SCRIPT_RUN=$(basename "$0")
...
# Check that required superuser environment variables are present.
check_required_superuser_env_vars() {
  local missing=0

  for var in DJANGO_SUPERUSER_USERNAME DJANGO_SUPERUSER_EMAIL DJANGO_SUPERUSER_PASSWORD; do
    if [[ -z "${!var}" ]]; then
      echo "Required environment variable ${var} is not set."
      missing=1
    fi
...
run_mathesar() {
  ...
  # Create superuser if the flag is provided.
  if [[ "${CREATE_SUPERUSER}" = true ]]; then
    if ! check_required_superuser_env_vars; then
        err "Required environment variables for superuser are not set."
    fi
    ...
    pushd "${BASE_DIR}" > /dev/null
      ...
      export ALLOW_FAILURE="true"
      run_cmd "$python_bin" manage.py createsuperuser --no-input
      unset ALLOW_FAILURE
    popd > /dev/null
  fi
...
Usage: ${SCRIPT_RUN} <command> [options]
